                ...
                {indexes.map((row_id, idx) => (
                    <div key={idx} className="flex-nowrap flex">
                        {indexes.map((square_id, idx) => (
                            <Square key={idx} square_id={square_id} row_id={row_id} handleHover={handleHover} />
                        ))}
                    </div>
                ))}
            </div>
            <div className='pl-5'>
                ...
                <div className='alerts'>
                    {alerts && alerts.length > 0 ? alerts.map((alert, idx) => (
                        <div key={idx} className="bg-amber-300 p-3 my-2 font-bold rounded-md">row {alert.row_id} square {alert.square_id}</div>
                    )) : ""}
                </div>
            </div>
        </div>
